...
- 각 세탁소별 포스기를 이용하는 사장님들의 맞춤형 데이터를 제공하고 
...
1. CORS 작동방식 중 `Credentials(자격 증명)` 방식을 통해 브라우저 요청 헤더에 쿠키(인증 토큰)를 포함하여 서버단에서 `JWT` 토큰을 검증하는 코드입니다.
2. `JWT(JSON Web Token)` 은(는) JSON 포맷을 이용하여 사용자에 대한 property를 저장(생성)하는 Claim 기반의 WebToken입니다.
3. `토큰-기반-인증` 시스템은 인증 받은 사용자들에게 토큰을 발급하고, 서버에 요청할 때마다 쿠키에 보관된 사용자 인증 정보를 검증하기 때문에, `서버-기반-인증` 방식 처럼 세션ID 토큰을 검증하기 위해 서버의 리소스를 많이 소모할 필요가 없습니다.
...
3. 비즈니스 로직과 UI로직을 따로 분리하기 위해 `useAuthFetch` 라는 Custom Hook을 만들었습니다.
